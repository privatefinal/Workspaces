...
import com.sun.mail.imap.IMAPStore;
	...
	public class ShowMail {
		...
		public static void main(String args[])  {
			String host = "imap.126.com"; // 邮件服务器
			String username = "18813907332@126.com";
			String password = "zhuange888";     // 126邮箱授权码
...
//            Message[] message = folder.getMessages(folder.getMessageCount()-folder.getUnreadMessageCount()+1,folder.getMessageCount());  
            ...
            Message message2[] = new Message[folder.getUnreadMessageCount()];
            int n = 0;
            for(int j = 0 ;j < message.length; j++){
            	Flags flag = message[j].getFlags();  
			...
			for (int i = 0; i < message2.length; i++) {
				// 获取未读邮件
				Flags flags = message2[i].getFlags();  
		      if (!flags.contains(Flags.Flag.SEEN)){
		    	  	re = new ShowMail((MimeMessage) message2[i]);
					...
					if(!re.isNew()){
						...
						System.out.println("邮件　" + i + "　是否包含附件:　"+ re.isContainAttach((Part) message2[i]));
						...
						re.getMailContent((Part) message2[i]);
						System.out.println("邮件　" + i + "　正文内容:　\r\n" + re.getBodyText());
						re.setAttachPath("e:\\126.com\\");
						re.saveAttachMent((Part) message2[i]);
						//设置邮件已读
//						message2[i].setFlag(Flags.Flag.SEEN, true);  
					} 	
